  ...
  context "rover starting at #{start_pos.x} #{start_pos.y} #{start_pos.direction}" do
    ...
    context "movement stream #{moves}" do
      it 'should finish on position #{end_pos.x} #{end_pos.y} #{end_pos.direction}' do
        ...
        expect(subject.position.y).to eq(end_pos.direction)
      end
    end
  end
end
...
describe Rover do
  ...
  it_should_behave_like 'rover movement',@plateu, Position.new(1, 2, 'N'),
    Position.new(1, 3, 'N'), 'LMLMLMLMM'
  it_should_behave_like 'rover movement',@plateu, Position.new(3, 3, 'E'),
